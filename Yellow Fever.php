...
<div id="change_class" class="flex_container section menu_section">
	...
	<div class="flex_item menu_item menu_item_outside"><a href="index.php"><p>Home</p></a></div>
		...
		$sql = mysqli_query($dbConn,"select * from ailments where Ailment = 'Yellow Fever'"); while($query = mysqli_fetch_array($sql)) {$intro = $query['Intro'];$symp = $query['Symptom'];$cause = $query['Cause'];$complic = $query['Complication'];$nmm = $query['Ailment'];
			}?>
