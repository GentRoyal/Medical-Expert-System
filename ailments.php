...
<?php
		$cid= (string)$_GET['cid'];
		include 'config.php';
		$sql = mysqli_query($dbConn,"select * from ailments where Ailment = '$cid'");
		while($query = mysqli_fetch_array($sql))
		{
			$intro = $query['Intro'];$symp = $query['Symptom'];$cause = $query['Cause'];$complic = $query['Complication'];$nmm = $query['Ailment'];;
		}?>
